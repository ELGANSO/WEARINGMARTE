...
abstract class AW_Blog_Block_Abstract extends Mage_Core_Block_Template
{
    ...
    protected function _processCollection($collection, $category = false)
    {
        ...
        foreach ($collection as $item) {
            ...
            // $exerptImage = self::$_helper->useShortContent();
            if ($item->getExerptImage()) {
                $item->setPostContent($item->getExerptImage());
            }
            if ($shortContent) {
    ...
    {
        return '<a class="aw-blog-read-more" href="' . $item->getAddress() . '"><span><span>' . $this->__('Read More') . '</span></span></a>';
    }
        ...
        if (self::$_helper->isEnabled()) {
            ...
            if ($this->getParentBlock()) {
                if ($type == self::LINK_TYPE_HEADER) {
                    $this->getParentBlock()->addLink($title, self::$_helper->getRoute(), $title, true, array(), 50, null,'class="top-link-blog"');
                } else {
    ...
    protected function _prepareData($item)
    {
        ...
        $item->setExerptImage(trim($item->getExerptImage()));
