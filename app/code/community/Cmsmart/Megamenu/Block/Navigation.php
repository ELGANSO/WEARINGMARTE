        ...
        $isOutermost = false, $outermostItemClass = '', $childrenWrapClass = '', $noEventAttributes = false)
    {
       ...
       $item_countersss = 1;
        ...
        foreach ($attributes as $attrName => $attrValue) {       
            $htmlLi .= ' ' . $attrName .  '="' . str_replace('"', '\"', $attrValue) . ' ' . $thumbnail . '"';
                  } 
        
        if($level == 1) {
                $idc = $category->getParentId();
                $widthp = $this->widthp($idc);
                $col_count = $this->getLevle($idc);
        ...
        // if($hasActiveChildren){
            // if($column > 1){
                // var_dump($column.' ');
                //var_dump($this->_getItemPosition($level));
               //  echo ($column.' - '.$childrenCount.'<br />');
        ...
        if($level == 1){    		
         $html[] = '<a style="'.($this->backgroundcolor($megamenu) ? ('background-color:'. $this->backgroundcolor($megamenu)).';' : '') . ' ' . ( $this->fontcolor($megamenu) ? ('border-bottom: 2px solid '.$this->fontcolor($megamenu)) : '') .'" class="catagory-level1" href="'.$this->getCategoryUrl($category).'"'.$linkClass.'>';        
        } else {
        ...
        if ($childrenWrapClass) {
            ...
            if($level == 0) {
                ...
                $html[] = '<div class="level' . $level .' '. $addclass .' column'.$this->getLevle($id).'" style="width:'.$width_menu. 'px;">';   
            } else {
                $html[] = '<div class="level' . $level .' '. $addclass .' column'.$this->getLevle($id).'">';    
            }
            ...
            if($level == 0) {
                ...
                $html[] = '<ul class=" level' . $level . $classblock.'"><li>'.$this->getBlockLeft($megamenu);
                  ...
                  if(@$megamenu[0]['category_children']==0)   {   
                    ...
                    if($htmlChildren) {
                        if($level == 0){
                            ...
                            $html[] = '<ul class="catagory_children"><li class="li-wrapper">'.$htmlChildren.'</li></ul>'; 
                            
                        } else {
                            $html[] = '<ul class="catagory_children column'.$this->getLevle($id).'"><li class="li-wrapper">'.$htmlChildren.'</li></ul>';    
                        }
                     }
                   }
            }  else{
				...
				$html[] = '<ul class="d level' . $level . $classblock.'">'.$this->getBlockLeft($megamenu);
                if(@$megamenu[0]['category_children']==0)   {   
                    if($htmlChildren) {
                        ...
                        if(isset($widthp)){
                            ...
                            if((@$widthc[0]['active_static_block'] == 1 ) && (@$widthc[0]['active_static_block_left'] == 1 ) && (@$widthc[0]['active_static_block_right'] == 1 )) {
                                ...
                                $html[] = '<li class="catagory_children '. $classchildren  .' column'.$this->getLevle($id).'" style="width:'.$widthchildrenc.'px">'.$htmlChildren.'</li>';
                                    ...
                                    $html[] = '<li class="catagory_children '. $classchildren  .' column'.$this->getLevle($id).'" style="width:'.$widthchildrenc.'px">'.$htmlChildren.'</div>';
                                } else{
                                    $classchildren = '';
                                    $html[] = '<li class="catagory_children '. $classchildren  .' column'.$this->getLevle($id).'">'.$htmlChildren.'</li>';   
                                }
            ...
            $html[] = '</div>'.$this->getBlockBottom($megamenu);
        ...
        ->addCategoryFilter(Mage::getModel('catalog/category')->load($id));
         ...
         foreach ($_productCollection as $_product){
            ...
            $reviews = Mage::getModel('review/review')
                ...
                $html .= $productBlock->getPriceHtml($_product, true);
                if($level==0){
